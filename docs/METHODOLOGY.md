...
| Protocol | Yields source | TVL source | User positions | Notes |
|---|---|---|---|---|
| Native PoX Stacking | Hiro Stacks API (`/v2/pox`, `/v2/pox_address`) | None — protocol is its own reference | Hiro Stacks API | Direct on-chain. Solo stacking. APY computed from trailing 12-cycle reward history |
| Zest | DefiLlama Yields | DefiLlama Protocol | _Coming in v0.3 — on-chain reader_ | Yields reflect DefiLlama's methodology and update cadence. |
| Granite | DefiLlama Protocol (TVL only) | DefiLlama Protocol | _Coming in v0.3 — on-chain reader_ | Yields not yet surfaced; Granite uses isolated-collateral markets that DefiLlama doesn't enumerate per-pool. |
...
- Zest and Granite yield/TVL now sourced from DefiLlama (replacing the originally-planned protocol APIs, which don't exist — both protocols are on-chain only).
- Cross-validation adds a `self-sourced` confidence state for protocols whose data already comes from DefiLlama.
- On-chain position readers for Zest and Granite scheduled for v0.3.
- Health score rubric v0.1.1: `asset-diversity` rule now considers positions in addition to wallet balances.
