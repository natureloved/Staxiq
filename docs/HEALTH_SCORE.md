...
The rubric is versioned (current: **0.1.1**). When we change it, the version bumps and historical scores remain reproducible.
...
Awards points for holding meaningful exposure to multiple distinct asset types — across both the wallet and any active positions. A user whose sBTC is supplied to Zest still has BTC asset-class exposure.
